...
doc_maxlen=25
import torch
dataset = [torch.load(f'data/CNN_bert_data/cnndm.train.{i}.bert.pt') for i in range(43)]
dataset = sum(dataset, [])
...
def make_batch(data):
    ...
    labels = tf.convert_to_tensor(_pad(pre_labels, 0))
    segs = tf.convert_to_tensor(_pad(pre_segs, 0))
    mask = ~(src == 0)
    clss = np.array(_pad(pre_clss, -1))
    mask_cls = ~(clss == -1)
    clss[clss == -1] = 0
