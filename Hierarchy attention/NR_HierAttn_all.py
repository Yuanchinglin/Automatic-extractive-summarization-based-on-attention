...
def make_batch(data):
    src_train = [data_train[i]['src_tensor'] for i in range(len(data_train))]
    pre_labels = [x['tgt'] for x in data]
    pre_clss = [x['clss'] for x in data]
    
    src = tf.ragged.stack(src_train).to_tensor(shape=(len(data_train), doc_maxlen, sent_maxlen))
    labels = tf.convert_to_tensor(pre_labels)
    mask = ~(src == 0)
    clss = np.array(_pad(pre_clss, -1, doc_maxlen))
    mask_cls = ~(clss == -1)
    clss[clss == -1] = 0
    return src, clss, mask, mask_cls, labels
...
@tf.function
def train_step(inp, targ, mask):
    loss = 0
    with tf.GradientTape() as tape:
        logit, _ = encoder(inp, True, mask)
